...
Do **not** put `BOT_TOKEN`, `BACKEND_API_KEY`, FCS API/Twelve Data keys, wallet private keys or seed phrases in the Mini App frontend.
...
```text
GET /api/admin/fcs/availability
GET /api/admin/cache
```
...
- FCS Forex/Metals current price: `FCS_PRICE_CACHE_SECONDS` (default 60 seconds)
- Twelve Data Crypto/Nasdaq current price: `PRICE_CACHE_SECONDS` (default 15 seconds)
...
6. Open the Mini App from Telegram and test `/api/me`, one FCS API signal and one Twelve Data signal.
