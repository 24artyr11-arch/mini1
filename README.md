...
The root `main.py` exposes one FastAPI/Uvicorn server. Telegram polling starts through FastAPI lifespan, so there is no second Uvicorn bind and no deprecated `@app.on_event` startup/shutdown handler.
