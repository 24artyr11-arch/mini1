...
def intro_keyboard(active: bool) -> InlineKeyboardMarkup:
    text = "OPEN SIGNALS" if active else "GET BOT"
    callback = "access:open" if active else "get_bot"
    return InlineKeyboardMarkup(inline_keyboard=[[green_button(text, callback)]])


def plan_text(settings: Settings) -> str:
    return PLAN_TEXT.format(monthly=settings.monthly_usd, lifetime=settings.lifetime_usd)


def plans_keyboard(settings: Settings) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [green_button(f"MONTHLY {settings.monthly_usd:g}$", "plan:monthly")],
            [green_button(f"LIFETIME {settings.lifetime_usd:g}$", "plan:lifetime")],
            [back_button("intro")],
        ]
    )
...
def payment_methods_keyboard(plan_key: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [green_button("USDT TRC20", f"payment:usdt:{plan_key}")],
            [green_button("BTC", f"payment:btc:{plan_key}")],
            [back_button("plans")],
        ]
    )
...
def amount_keyboard(currency: str, plan_key: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [green_button("Get the address for payment", f"address:{currency}:{plan_key}")],
            [back_button(f"payment_methods:{plan_key}")],
        ]
    )
...
def open_signals_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[[green_button("📊 Open Signals", "access:open")]]
    )
...
async def show_intro_message(message: Message, store: AccessStore, settings: Settings) -> None:
    ...
    await message.answer(
        ...
        reply_markup=intro_keyboard(active),
    )
...
async def intro_callback(callback: CallbackQuery, access_store: AccessStore, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    active = has_access(callback.from_user.id, access_store, settings)
...
@router.callback_query(F.data == "get_bot")
async def get_bot(callback: CallbackQuery, access_store: AccessStore, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    if has_access(callback.from_user.id, access_store, settings):
        await callback.message.edit_text(
            "<b>Market Signal Bot</b>\n\nChoose a market:",
            parse_mode=ParseMode.HTML,
...
async def plans(callback: CallbackQuery, settings: Settings) -> None:
    await callback.answer()
    if callback.message:
        await callback.message.edit_text(
            plan_text(settings),
...
async def choose_plan(callback: CallbackQuery) -> None:
    ...
    if plan_key not in {"monthly", "lifetime"}:
        return
    await callback.message.edit_text(
...
@router.callback_query(F.data.startswith("payment_methods:"))
async def back_to_payment_methods(callback: CallbackQuery) -> None:
    await callback.answer()
    if not callback.message:
        return
    plan_key = callback.data.split(":", 1)[1]
...
async def payment_method(callback: CallbackQuery, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    ...
    if not amount:
        ...
        return
    await callback.message.edit_text(
        f"Pay the amount: <b>{escape(amount)}</b>",
        parse_mode=ParseMode.HTML,
        reply_markup=amount_keyboard(currency, plan_key),
    )
...
async def show_payment_address(
    ...
    if not wallet:
        await callback.answer(
            "This payment method is not configured. Contact support.",
            show_alert=True,
        )
        return
    ...
    await callback.message.edit_text(
        ...
        "After payment, send the payment receipt to this bot as a <b>photo or document</b>.",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[back_button(f"payment_methods:{plan_key}")]]
        ),
    )
    ...
    await bot.send_message(
        callback.from_user.id,
        "📎 <b>Now send your payment receipt here.</b>\n\n"
        "You can send a screenshot/photo or a document. Your receipt will be forwarded to the administrator for manual verification.",
        parse_mode=ParseMode.HTML,
    )
        ...
        if updated:
            try:
                ...
                await bot.send_message(
                    ...
                    parse_mode=ParseMode.HTML,
                    reply_markup=open_signals_keyboard(),
                )
            except (TelegramForbiddenError, TelegramBadRequest):
                pass
            await callback.message.edit_text(
...
async def payment_receipt(
    ...
    if message.from_user is None:
        return
    intent = access_store.get_intent(message.from_user.id)
    if not intent:
        return
    ...
    # Copy the actual receipt first, then send a review card with admin actions.
    try:
        await bot.copy_message(
        ...
        logger.exception("Could not copy payment receipt to admin")

    username = f"@{message.from_user.username}" if message.from_user.username else "—"
    plan_name = "30 Days" if intent["plan_key"] == "monthly" else "Lifetime"
    currency_name = "USDT TRC20" if intent["currency"] == "usdt" else "BTC"
    await bot.send_message(
        settings.admin_telegram_id,
        ...
        f"Метод: <b>{currency_name}</b>\n"
        f"Ожидаемая сумма: <b>{escape(intent['amount_text'])}</b>\n\n"
    ...
    await message.answer(
        ...
        "Your payment has been sent to the administrator for verification. You will receive a message here after it is reviewed.",
        parse_mode=ParseMode.HTML,
    )
...
async def admin_approve(
    ...
    if callback.message:
        await callback.message.edit_text(
            ...
            + f"\n\n✅ <b>ПОДТВЕРЖДЕНО</b> администратором <code>{callback.from_user.id}</code>\nДоступ: <b>{escape(expiry_text)}</b>",
            parse_mode=ParseMode.HTML,
        )
...
async def admin_reject(
    ...
    try:
        await bot.send_message(
            ...
            "Please check your transaction details and submit a new receipt if needed.",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[green_button("Choose Plan", "get_bot")]]
            ),
        )
    except Exception:
    ...
    if callback.message:
        await callback.message.edit_text(
            callback.message.html_text
            + f"\n\n❌ <b>ОТКЛОНЕНО</b> администратором <code>{callback.from_user.id}</code>",
            parse_mode=ParseMode.HTML,
        )


@router.callback_query(F.data == "access:open")
async def access_open(callback: CallbackQuery, access_store: AccessStore, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    if not has_access(callback.from_user.id, access_store, settings):
        await callback.message.edit_text(
            plan_text(settings),
            parse_mode=ParseMode.HTML,
            reply_markup=plans_keyboard(settings),
...
async def home(callback: CallbackQuery, access_store: AccessStore, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    if not has_access(callback.from_user.id, access_store, settings):
...
async def market_selected(callback: CallbackQuery, access_store: AccessStore, settings: Settings) -> None:
    await callback.answer()
    if not callback.message:
        return
    if not has_access(callback.from_user.id, access_store, settings):
        ...
        return
    market = _parse_market(callback.data.split(":", 1)[1])
    if market is None:
        return
    await callback.message.edit_text(
        f"<b>{MARKET_TITLES[market]}</b>\n\nChoose an instrument:",
        parse_mode=ParseMode.HTML,
        reply_markup=instruments_keyboard(market),
    )


@router.callback_query(F.data.startswith("asset:"))
async def asset_selected(
    callback: CallbackQuery,
    ...
    if not has_access(callback.from_user.id, access_store, settings):
        await callback.answer("Access required.", show_alert=True)
        if callback.message:
            await callback.message.edit_text(
                plan_text(settings),
                parse_mode=ParseMode.HTML,
        ...
        return
    await callback.answer("Analyzing M15 market data…")
    parts = callback.data.split(":")
    if len(parts) != 3 or callback.message is None:
        return
    market = _parse_market(parts[1])
    if market is None:
        return
    instrument = get_instrument(market, parts[2])
    if instrument:
        await _calculate_and_render(callback, instrument, signal_service, signal_cache)


@router.callback_query(F.data.startswith("signal:"))
async def recalculate_signal(
    callback: CallbackQuery,
    signal_service: SignalService,
    signal_cache: SignalCache,
    access_store: AccessStore,
    ...
    if not has_access(callback.from_user.id, access_store, settings):
        ...
        return
    await callback.answer("Recalculating…")
    parts = callback.data.split(":")
    if len(parts) != 3 or callback.message is None:
        return
    market = _parse_market(parts[1])
...
async def _calculate_and_render(
    callback: CallbackQuery,
    instrument: Instrument,
    signal_service: SignalService,
    signal_cache: SignalCache,
    ...
    if callback.message is None:
        return
    try:
        signal = await signal_service.calculate(instrument)
        signal_cache.put(callback.from_user.id, instrument, signal)
        await callback.message.edit_text(
            format_signal(signal),
            parse_mode=ParseMode.HTML,
            reply_markup=signal_keyboard(instrument.market, instrument.id),
        )
    except TwelveDataRateLimitError:
        logger.warning("Twelve Data rate limit reached")
        await callback.message.edit_text(
            f"<b>{escape(RATE_LIMIT_MESSAGE)}</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=instruments_keyboard(instrument.market),
        )
    except MarketDataRateLimitError:
        logger.warning("Market data rate limit reached")
        await callback.message.edit_text(
            "<b>Market data is temporarily rate limited. Please try again shortly.</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=instruments_keyboard(instrument.market),
        )
        ...
        await callback.message.edit_text(
            "<b>Unable to calculate the signal.</b>\n\n"
            "Market data is temporarily unavailable for this instrument. Please try again later.",
            parse_mode=ParseMode.HTML,
            reply_markup=instruments_keyboard(instrument.market),
        )
...
async def refresh_price(
    ...
    signal = signal_cache.get(callback.from_user.id, instrument)
    if signal is None:
        await callback.answer("Signal cache expired. Recalculate the signal.", show_alert=True)
        return

    try:
        live = await signal_service.current_price(instrument)
        await callback.message.edit_text(
            format_price_update(signal, live),
            parse_mode=ParseMode.HTML,
            reply_markup=signal_keyboard(instrument.market, instrument.id),
        )
    except TwelveDataRateLimitError:
        logger.warning("Twelve Data rate limit reached during price refresh")
        await callback.answer(RATE_LIMIT_MESSAGE, show_alert=True)
    except MarketDataRateLimitError:
        logger.warning("Market data rate limit reached during price refresh")
        await callback.answer("Market data is temporarily rate limited. Please try again shortly.", show_alert=True)
    except MarketDataError as exc:
        logger.exception("Price refresh failed")
        await callback.answer("Market data is temporarily unavailable. Please try again later.", show_alert=True)
